...
本项目用Rust语言分阶段实现一个简单的大模型推理程序。

分为两个阶段：初级阶段：实现大模型的几个关键算子，Feed-Forward神经网络，以及大模型的参数加载；项目阶段：实现大模型最为核心的Self-Attention结构，完成大模型的文本生成功能。最后，继续实现AI对话功能，搭建一个小型的聊天机器人服务。

- 本项目支持Llama、Mistral及其同结构的Transformer模型，所使用的数据类型为FP32，使用CPU进行推理。
...
- 项目包含github on-push自动检测
- 在指定文件中和位置添加代码，不要修改其他文件和函数、文件名称和项目结构。该阶段不需要额外的第三方依赖。
- 不要修改已有的测试代码。开发过程中如果有需要，可以添加测例。
- 调试代码时，可以打印张量的数据，也可以使用pytorch中的函数辅助调试。
...
如果完成了项目，请向导师展示你的成果吧！其实这个项目还有很多可以拓展的地方，比如其他数据类型的支持、多会话的支持、GPU加速等等，欢迎你继续探索。
